...
int dijkstra(int **graph, int src, int dst, int V, int *node, int path_num, int flag, int req_ID, int **Fix_graph){
//    if(flag == 1){
//        printf("node_num : ");
...
//            break;
        ...
        for (int v = 0; v < V; v++){
            ...
            if(!sptSet[v] && dist[u]+1<dist[v] && Fix_graph[u][v]>0 && dist[u]!=INT_MAX){
                    dist[v] = dist[u] + 1;
                    parent[v] = u;
//                    if(flag ==1)
//                        printf("%d %d\n", v, dist[v]);
...
int main(){
	...
	int path_num = 0;
	for(int count=0; count<req_num; count++){
        path_num = dijkstra(cpy, req_src[count], req_dst[count], node_num, node_cpy, path_num, 0, count, graph);
	}
	//
...
//        node_cpy[count1] = node[count1];
    ...
    path_num = 0 ;
    for(int count=0; count<req_num; count++){
        path_num = dijkstra(graph, req_src[count], req_dst[count], node_num, node, path_num, 1, count, cpy);
    }
    //clear pointer
    for(int count=0; count<node_num; count++){
