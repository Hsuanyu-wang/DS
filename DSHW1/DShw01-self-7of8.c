...
int main(){
	...
	for(int count=0; count<req_num; count++){
        path_num1 = dijkstra(cpy, req_src[count], req_dst[count], node_num, node_cpy, path_num1, 0, count);
	}
    printf("%d\n", path_num1);

    //test mindistance_descend path_num
    for(int count=0; count<req_num; count++){
        path_num2 = dijkstra(graph, req_src[count], req_dst[count], node_num, node, path_num2, 1, count);
    }
