...
int minDistance_ascend(int dist[], bool sptSet[], int V){//for func dijkstra
...
int minDistance_descend(int dist[], bool sptSet[], int V){//for func dijkstra
    // Initialize min value
    int min = INT_MAX, min_index = -3;
//    int min_index = -1;
    for (int v = V-1; v >=0; v--)
...
int dijkstra(int **graph, int src, int dst, int V, int *node, int path_num, int flag, int req_ID, int print_ctrl){
//    if(print_ctrl == 1){
//        printf("node_num : ");
...
//            parent[count][count1] = -2;
    ...
    for (int count = V-1; count >= 0; count--){
        ...
        int u = -3;
        if(flag == 0)
            u = minDistance_ascend(dist, sptSet, V);
        else if(flag == 1)
            u = minDistance_descend(dist, sptSet, V);
//        if(print_ctrl == 1)
//            printf("u = %d\n", u);
...
int main(){
	...
	//紀錄成功的路徑數
	int path_num1 = 0, path_num2 = 0;

    //test mindistance_ascend path_num
	for(int count=0; count<req_num; count++){
        path_num1 = dijkstra(cpy, req_src[count], req_dst[count], node_num, node_cpy, path_num1, 1, count, 0);
	}
	//recover cpy % node_cpy
    for(int count1 = 0; count1 < node_num; count1++){
        node_cpy[count1] = node[count1];
        for(int count2 = 0; count2 < node_num; count2++){
            cpy[count1][count2] = graph[count1][count2];
        }
    }
    //test mindistance_descend path_num
    for(int count=0; count<req_num; count++){
        path_num2 = dijkstra(graph, req_src[count], req_dst[count], node_num, node, path_num2, 0, count, 0);
    }
    //recover graph & node
    for(int count1 = 0; count1 < node_num; count1++){
        node[count1] = node_cpy[count1];
        for(int count2 = 0; count2 < node_num; count2++){
            graph[count1][count2] = cpy[count1][count2];
        }
    }
    //print answer
    int path_num = 0;
    if(path_num1 >= path_num2){
        printf("%d\n", path_num1);
        for(int count=0; count<req_num; count++){
            path_num = dijkstra(cpy, req_src[count], req_dst[count], node_num, node_cpy, path_num, 1, count, 1);
        }
    }
//printf("###############################################\n");
//    path_num = 0;
        ...
        printf("%d\n", path_num2);
        for(int count=0; count<req_num; count++){
            path_num = dijkstra(graph, req_src[count], req_dst[count], node_num, node, path_num, 0, count, 1);
        }
    }
