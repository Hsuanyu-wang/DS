...
int main(){
    ...
    ////edge是可用臨界
    //int edge_top = rsrc_y, edge_down = 0
    ...
    for(int i = 0; i < user_num; i++){
        ...
        while(current != NULL){
            if(current->freq == 16 && current->time == 1 && (edge_right - edge_left) >= 1){// 16 * 1, set position
                list_x[i] = --edge_right;
                list_y[i] = 0;
                accept_num++;
                data_head[i] = current;
        ...
        //判斷 other case 直接落下
        if(data_head[i]->time <= (edge_right - edge_left) && list_x[i] == -1 && edge_left < edge_right){// accept
            list_x[i] = edge_left;
            list_y[i] = 0;
