...
double **make2Darray(int row, int col, double* node_x, double* node_y){//for graph
...
//            else
                temp[count1][count2] = sqrt( dist_count(node_x[count1], node_y[count1], node_x[count2], node_y[count2]) );
        }
    }
    return temp;
}
int **make2Darray_int(int row, int col){//for graph
    double **temp = malloc(sizeof(int*) * row);
    for(int count=0; count<row; count++){
...
int main(){
    ...
    //讀取點座標 & 初始化陣列
    int skipID, sptSet[node_num], djtSet[node_num], parent[node_num];
    double node_x[node_num], node_y[node_num], **dist;
    for(int count = 0; count < node_num; count++){
    ...
    dist = make2Darray(node_num, node_num, node_x, node_y);
    //印出初始矩陣
    printf("dist (2D)\n");
    ...
    int **graph = make2Darray_int(node_num, node_num);
    printf("GG\n");
    print_2Darray_int(graph, node_num, node_num);
    printf("GG\n");
    kruskal_MST(node_num, heap, djtSet, sptSet, graph);
    print_2Darray_int(graph, node_num, node_num);
    //對graph 做 dijkstra
    //int dijkstra(int **graph, int src, int dst, int VN, int *node, int path_num, int flag)
    for(int count = 0; count < node_num; count++){
        free(dist[count]), free(graph[count]);
    }
    free(dist), free(graph), free(heap);
    printf("end\n");
	return 0;
}
